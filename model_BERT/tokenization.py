...
def convert_tokens_to_ids(tokens):
    ...
    for token in tokens:
        try:
            ids.append(embed_lookup.wv.vocab[token].index)
        except:
            ids.append(0)
    return ids

def convert_tokens_to_ids1(vocab, tokens):
    """Converts a sequence of tokens into ids using the vocab."""
    ids = []
    for token in tokens:
...
def convert_tokens_to_ids2(tokens):
    """Converts a sequence of tokens into ids using the vocab."""
    ids = []
    for token in tokens:
        ids.append(token)
...
class FullTokenizer(object):
    ...
    def convert_tokens_to_ids2(self, tokens):
        return convert_tokens_to_ids2(tokens)
    
    def convert_to_unicode(self, text):
        return convert_to_unicode(text)
    
    def convert_idx_to_token(self, index):
        idx2token = {val : key for key,val in self.vocab.items()}
        return idx2token[index]
    

class FullTokenizer1(object):
    """Runs end-to-end tokenziation."""
    ...
    def convert_tokens_to_ids1(self, tokens):
        return convert_tokens_to_ids1(self.vocab, tokens)
